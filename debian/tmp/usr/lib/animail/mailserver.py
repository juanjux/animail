...
#--------------------------------------------------
        ...
        def get_header(self, nummsg, left): pass
        def stats(self): pass
        def clean(self): pass
 #--------------------------------------------------
        def finished(self):
...
#------------------------------------------------------
        def look_message(self,msg, logAgent, indice_log, left, smtpobj):
            ...
            if self.config.optsize and (msg['size'] >= self.config.max_size):
                    aprint(_("Message %d size exceed limit (%d), deleting it") % (msg['number'], self.config.max_size), general.RED)
                    if not optnotlog:
            ...
            listacab = self.get_header(num_msg, left)
            ...
            if self.config.optaccept:
                    ...
                    if self.config.notInAccept == 'delete':
                        aprint(_("Deleting message %d: Don't match anything in accept file") % num_msg,general.RED)
                        if not optnotlog:
                        ...
                        if not isConfirmation: laterReply = 1 #Gives the opportunity to filter it
            ...
            for comp in lfilterscomp:
                    ...
                    if result is not None:
                        ...
                        aprint(_("Deleting message %d: Header match %s") % (num_msg, patron), general.RED)
            ...
            if laterReply:
                aprint(_("Message %d (from %s) not in accept file: postergating it"\
                                    %(num_msg, fromwho)), general.VIOLET)
                if not optnotlog: 
