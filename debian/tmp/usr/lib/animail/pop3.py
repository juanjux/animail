...
#--------------------------------------------------
    def get_header(self, nummsg, left):
        aprint(_("Looking header %d (%s)") % (nummsg, self.alias), general.VIOLET)
        header = self.objpop.top(int(nummsg), 0)
        return header
    # end def get_header
#--------------------------------------------------
    def connect(self):
            ...
            if not optv:
                ...
                if not optsyslog and not optquiet:
                    ...
                    if iter % 5 == 0:
                        ...
                        strtmp2 = general.colorize("%3.0f%%" % (total / (msg_size * 1.0) * 100.0), general.WHITE)
