...
import time, syslog, sys, locale, os, re, rfc822, StringIO, textwrap
...
anti-publicidad que  se ejecuta  en mi ordenador.  Su propósito  es evitar
...
los  mensajes que  envée en  un futuro  a mi  dirección serÃ¡n  aceptados
...
anti-pubblicitÃ¡ eseguito dal  mio computer,al fine sia  di evitare posta
...
[Si desea saber mÃ¡s sobre Animail visite http://escomposlinux.org/fer_y_juanjo/animail.html]
[Se desidera ulteriori informazioni sull'Animail visiti http://animail.sf.net]
"""

ConfirmDefaultStr = """
[En espaÃ±ol mÃ¡s abajo]
[In italiano in seguito]
...
[EspaÃ±ol]
-------------------------------------------------------------------------
Su mensaje o mensajes han sido  entregados, y  usted ha sido aÃ±adido a mi
lista de aceptados automÃ¡ticamente. Gracias.

[Italiano]
-------------------------------------------------------------------------
Il suo messaggio o messaggi sono stati ricevuti, e lei Ã© stato aggiunto
...
def get_term_width():
    try:
        # Should work in most posix systems
        cmd = os.popen('tput cols')
        wth = int(cmd.read())
        cmd.close()
        return wth
    except:
        ...
        return 80        
#-------------------------------------------------------------------
def aprint(str, color,prioridad=syslog.LOG_NOTICE, iserror=False):
        """print to the standar output, error or system log, wrap text and and apply colors"""
        if Application.config.optcheck: return
        if Application.config.optsyslog: 
            syslog.syslog(prioridad, str)        
            return
        #FIXME: Uncomment this line!
        #str = textwrap.fill(str,Application.config.term_width)        
        if iserror: sys.stderr.write(str+'\n')
        else:
