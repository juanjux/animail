...
class AnimailMain(object):
        ...
        def process (self, serv):
            ...
            # Ok, this is not very elegant but the thread really needs to know if a lock
            # has been created by himself (and thus can be safely released) or by another
            # thread to release it if we get an exception:
            own_lock = 0
...
#-EXCEPTION HANDLERS
        ...
        # end for
            ...
            # Just in case some exception was triggered before we could release the lock...
            if own_lock: self.release_lock()
        ...
        def release_lock(self):
            if self.CLock.locked(): self.CLock.release()
